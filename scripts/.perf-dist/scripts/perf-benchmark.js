...
import { parseExcelBuffer, writeExcelToArrayBuffer } from '../src/lib/excel.js';
import { transformData } from '../src/lib/transform.js';
...
function writeExcelBaseline(data) {
    ...
    const colWidths = Object.keys(data[0] || {}).map((key) => {
        const maxLen = Math.max(key.length * 2, ...data.map((row) => (row[key] || '').length * 1.5));
...
async function runBenchmarkForRowCount(rowCount) {
    ...
    const steps = [];
    const readResult = await measureStep(rowCount, 'readExcel(core)', () => parseExcelBuffer(inputBuffer));
    steps.push(readResult.stats);
    if (!readResult.value) {
        return steps;
    }
    const transformResult = await measureStep(rowCount, 'transformData', () => transformData(readResult.value.rows, config));
    steps.push(transformResult.stats);
    if (!transformResult.value) {
        return steps;
    }
    const writeOptimizedResult = await measureStep(rowCount, 'writeExcel(optimized)', () => writeExcelToArrayBuffer(transformResult.value));
    steps.push(writeOptimizedResult.stats);
    const writeBaselineResult = await measureStep(rowCount, 'writeExcel(baseline)', () => writeExcelBaseline(transformResult.value));
...
function printSummary(allStats) {
    ...
    console.table(table);
    const optimized = allStats.filter((s) => s.step === 'writeExcel(optimized)' && s.status === 'ok');
    const baseline = allStats.filter((s) => s.step === 'writeExcel(baseline)' && s.status === 'ok');
    if (optimized.length > 0 && baseline.length > 0) {
        ...
        for (const base of baseline) {
            ...
            console.log(`  ${base.rowCount.toLocaleString()} rows: ${formatNumber(speedup)}x faster (baseline ${formatNumber(base.durationMs)} ms -> optimized ${formatNumber(opt.durationMs)} ms)`);
        }
    }
}
async function main() {
